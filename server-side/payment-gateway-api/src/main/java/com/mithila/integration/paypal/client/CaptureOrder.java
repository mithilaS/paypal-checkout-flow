...
public class CaptureOrder extends PayPalClient {
    ...
    public HttpResponse<Order> captureOrder(String orderId, boolean debug) throws IOException {
        ...
        if (debug) {
            ...
            System.out.println("\tEmail Address: " + payer.email());
            System.out.println("\tName: " + payer.name().fullName());
  //          System.out.println("\tPhone Number: " + payer.phoneWithType().phoneNumber().countryCallingCode() + payer.phoneWithType().phoneNumber().nationalNumber());
        }
        return response;
    }
    ...
    public static void main(String[] args) {
        try {
            new CaptureOrder().captureOrder("3T1798259J101472K", true);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
